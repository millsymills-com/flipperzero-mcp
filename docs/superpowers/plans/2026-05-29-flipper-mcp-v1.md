...
`FLIPPER_USB_BAUDRATE` config value (default 115200) has no effect over USB. The
harvested ESP32 WiFi bridge sets its UART to 115200 (`CONFIG_BRIDGE_UART_BAUD_RATE`
in `firmware/tcp_uart_bridge/sdkconfig.defaults`), negotiating up from a 9600
start; no 230400 path exists in this repo.
