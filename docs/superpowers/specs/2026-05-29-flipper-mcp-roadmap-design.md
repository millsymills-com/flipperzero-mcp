...
- `protobuf_rpc.py` implements **only the CLI→RPC switch** (`_ensure_rpc_session_started`
  ...
  (`flipper.proto:58` for the message; `stop_session = 19` is field 19 of `Main`
  at line 67) and then draining to the `>:` prompt.
...
- **Resources** (bundled markdown under `src/flipperzero_mcp/resources/`, served
  ...
  - `flipper://reference/connection` — ports; the USB-CDC baud nuance (CDC ACM
    ...
    by the device. The harvested ESP32 bridge runs its UART at 115200
    (`CONFIG_BRIDGE_UART_BAUD_RATE` in `firmware/tcp_uart_bridge/sdkconfig.defaults`),
    negotiating up from a 9600 start; there is no 230400 path in this repo);
    CLI↔RPC mode switching incl. `StopSession`; which transports support which mode.
