...
def register_prompts(mcp: FastMCP) -> None:
    ...
    def flipper_doctor() -> str:
        ...
        return (
            ...
            "firmware flavor, channel/version, and `sd_card_available`.\n"
    ...
    def flipper_install(github_url: str) -> str:
        ...
        return (
            ...
            "(`application.fam`).\n"
            "3. Read `firmware.flavor` from `flipperzero_system_info`. For official firmware, "
            "run `ufbt update --channel=<release|rc|dev>` matching the device version; for "
            "Momentum, point ufbt at the Momentum SDK index per its docs. If "
            "`firmware.flavor` is unknown/unsupported or the repo targets a different "
            "channel/firmware, STOP and report — never produce an incompatible `.fap`.\n"
